...
q=["DO YOU HAVE NERVOUS FEELINGS","DO YOU HAVE RESTLESSNES OR TENSION",
   "DO YOU HAVE A SENSE OF IMPENDING PANIC  OR DANGER","DO YOU HAVE DEHYDRATION",
   "DO YOU HAVE AN INCREASED HEART RATE ","DO YOU HAVE RAPID BREATHING",
   "DO YOU HAVE SWEATING","DO YOU HAVE PALPITATIONS"]
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF ANXIETY. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                    response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                    if response==1:
                             root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF ANXIETY. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                    response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                    if response==1:
                             root.destroy()
...
yay=messagebox.showinfo("CAUSE FOR ANXIETY:"," There are many causes which include stress,pressure ,drug alcohol etc.\n NOT TRANSMISSIBLE")
