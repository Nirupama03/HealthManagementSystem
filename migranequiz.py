...
q=["DO YOU HAVE INTENSE THROBBING HEADACHE","DO YOU HAVE INTENSE THROBBING HEADACHE",
               "DO YOU HAVE OCCASSIONAL NAUSEA","DO YOU HAVE VOMITING"]
...
class Quiz:
    ...
    def create_q(self,master,qn):
        w=tk.Label(master,text=q[qn],font=("Comic Sans MS","35" ," bold"),bg="#EE7621")
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF MIGRANE. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF MIGRANE. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
...
yay=messagebox.showinfo("CAUSE FOR MIGRANE:",'''Bright lights and sun glare can induce migraines, ascan loud sounds.
Strong smells — including perfume, paint thinner, secondhand smoke and others — trigger migraines \n
NOT TRANSMISSIBLE ''')
