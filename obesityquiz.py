...
q=["DO YOU HAVE BACK PAIN","DO YOU HAVE JOINTPAIN","ARE YOU OVERWEIGHT",
   "DO YOU BINGE EAT","DO YOU HAVE FATIGUE","DO YOU HAVE POTBELLY","DO YOU SNORE WHILE SLEEPING"]
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF OBESITY. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF OBESITY. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
...
yay=messagebox.showinfo("CAUSE FOR OBESITY:",'''  Obesity is generally caused by eating too much and moving too little
NOT TRANSMISSIBLE''')
