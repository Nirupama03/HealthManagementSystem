...
q=["DO YOU HAVE SEVERE DIAORHOEA","DO YOU HAVE NAUSEA","DO YOU HAVE ABDOMINAL PAIN","DO YOU HAVE DEHYDRATION","DO YOU HAVE VOMITING"]
...
class Quiz:
    ...
    def create_q(self,master,qn):
        w=tk.Label(master,text=q[qn],font=("Comic Sans MS","40" ," bold"),bg="#EE7621")
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF CHOLERA. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                             root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF CHOLERA. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                             root.destroy()
...
yay = messagebox.showinfo("CAUSE FOR CHOLERA:",'''  A bacterium called Vibrio cholerae
Cholera is highly contagious. Cholera can be transferred person to person by infected fecal matter entering
a mouth or by water or food contaminated with Vibrio cholerae bacteria.''')
