...
q=["DO YOU HAVE HEADACHE","DO YOU HAVE STIFF NECK","DO YOU HAVE BACK PAIN",
   "DO YOU HAVE BLOTCHY OR RED RASHES","DO YOU HAVE  FEVER AND CHILLS",
   "DO YOU HAVE LOSS OF APPETITE","DO YOU HAVE IRRITABILITY",
   "DO YOU HAVE SENSITIVITY OF LIGHT"]


options =[["yes","no"],["yes","no"],["yes","no"],["yes","no"]
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF MENINGITIS. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF MENINGITIS. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
...
yay=messagebox.showinfo("CAUSE FOR MENINGITIS",''' The most common causes of meningitis are viral and bacterial infections
