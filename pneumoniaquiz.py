...
q=["DO YOU HAVE SEVERE FEVER","DO YOU HAVE SWEATING","DO YOU HAVE SHAKING CHILLS",
   "DO YOU HAVE LACK OF BREATH","DO YOU HAVE RAPID AND SHALLOW BREATHING",
   "DO YOU HAVE CHEST PAIN WHICH WORSENS","DO YOU HAVE LOSS OF APEITE",
   "DO YOU HAVE FATIGUE"]
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF PNEUMONIA. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                        response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                        if response==1:
                             root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF PNEUMONIA. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
...
yay=messagebox.showinfo("CAUSE FOR PNEUMONIA:",'''  Caused by Bacterial pneumonia
 It's usually caused by an infection, most commonly bacteria and viruses, which are both contagious.''')
