...
q=["DO YOU HAVE AGITATION,IRRITABILITY,HOSTILITY","DO YOU HAVE  ANY PREVIOUS HISTORY"
   ,"DO YOU HAVE NIGHTMARES:","DO YOU HAVE AVOID SITUATIONS WHICH BRING BACK TRAUMA  "
   ,"DO YOU HAVE HEIGHTENED REACTIVITY TO STIMULI ","DO YOU HAVE ANXIETY OR DEPRESSION ",
   "DO YOU HAVE LOSS OF PLEASURE IN ACTIVITIES","DO YOU HAVE INSOMNIA","DO YOU HAVE EMOTIONAL DETACHMENT"]


options =[["yes","no"],["yes","no"],["yes","no"],["yes","no"],["yes","no"],["yes","no"],["yes","no"],["yes","no"],["yes","no"]]
...
class Quiz:
    ...
    def create_q(self,master,qn):
        w=tk.Label(master,text=q[qn],font=("Comic Sans MS","25" ," bold"),bg="#EE7621")
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF POST TRAUMATIC STRESS DISORDER. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                      response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                      if response==1:
                             root.destroy()
                ...
                hui =tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF POST TRAUMATIC STRESS DISORDER . PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
...
yay=messagebox.showinfo("CAUSE FOR PTSD:",'''  Several factors like harsh circumstances and history that an individual cannot withstand at that point of time.\n
NOT TRANSMISSIBLE ''')
