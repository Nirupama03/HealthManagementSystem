...
q=["DO YOU HAVE  COUGH LASTING A WEEK","DO YOU COUGH BLOOD","DO YOU HAVE CHEST PAIN",
               "DO YOU HAVE PAIN WHILE BREATHING","DO YOU HAVE PAIN WHILE COUGHING"]
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF TUBERCULOSIS. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                    response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                    if response==1:
                                    root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF TUBERCULOSIS. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                    response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                    if response==1:
                          root.destroy()
