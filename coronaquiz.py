...
q=["DO YOU HAVE SEVERE FEVER","DO YOU HAVE TIREDNESS",
   "DO YOU HAVE DRYCOUGH","DO YOU HAVE ACHE&PAIN","DO YOU HAVE NASALCONGESTION",
   "DO YOU HAVE DIARRH0EA","DO YOU HAVE CONJUCTIVITIES",
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF CORONAVIRUS. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                        response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                        if response==1:
                                    root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF CORONAVIRUS. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                    response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                    if response==1:
                              root.destroy()
...
yay = messagebox.showinfo("CAUSE FOR CORONA:",'''The virus that causes COVID-19 is in a family of viruses called Coronaviridae. 
