...
q=["DO YOU HAVE FATIGUE","DO YOU HAVE ABDOMINAL PAIN","DO YOU HAVE WEIGHT LOSS",
               "DO YOU HAVE VOMITING","DO YOU HAVE FEVER"]
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF  JAUNDICE. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF JAUDICE. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                         root.destroy()
...
yay=messagebox.showinfo("CAUSE FOR JAUNDICE",'''  Caused by excess bilirubin
Jaundice does not pose any infection risks and isn't communicable on its own.''')
