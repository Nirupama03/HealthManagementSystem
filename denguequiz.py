...
q=["DO YOU HAVE FEVER?","DO YOU HAVE SEVERE JOINT AND MUSCLE PAIN?","RASHES AFTER 2 DAYS OF FEVER",
                   "SWOLLEN LUMP NODES","HEADACHE"]
    ...
    def next_btn(self):
        ...
        if self.qn == len(q):
            if self.correct>=3:
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU DON'T HAVE  ALL THE SYMPTOMS OF DENGUE. BUT IT WOULD BE GOOD IF YOU  CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                             root.destroy()
                ...
                hui=tk.messagebox.showinfo("RESULT","YOU HAVE SYMPTOMS OF DENGUE. PLEASE CONSULT A DOCTOR")
                if hui=="ok":
                     response=messagebox.askyesno("Feedback","WE VALUE YOUR FEEDBACK !!")
                     if response==1:
                             root.destroy()
...
yay = messagebox.showinfo("CAUSE FOR DENGUE:",''' Caused by a family of viruses transmitted by Aedes mosquitoes
